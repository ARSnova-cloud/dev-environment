...
CONTROLLER_CLOUD_CONFIG_PATH = File.expand_path("./kubernetes/generic/controller-install.sh")
WORKER_CLOUD_CONFIG_PATH = File.expand_path("./kubernetes/generic/worker-install.sh")
...
# Generate root CA
system("mkdir -p ssl && ./kubernetes/lib/init-ssl-ca ssl") or abort ("failed generating SSL artifacts")

# Generate admin key/cert
system("./kubernetes/lib/init-ssl ssl admin kube-admin") or abort("failed generating admin SSL artifacts")

def provisionMachineSSL(machine,certBaseName,cn,ipAddrs)
  tarFile = "ssl/#{cn}.tar"
  ipString = ipAddrs.map.with_index { |ip, i| "IP.#{i+1}=#{ip}"}.join(",")
  system("./kubernetes/lib/init-ssl ssl #{certBaseName} #{cn} #{ipString}") or abort("failed generating #{cn} SSL artifacts")
...
Vagrant.configure("2") do |config|
  ...
  (1..$etcd_count).each do |i|
    config.vm.define vm_name = "e%d" % i do |etcd|
      ...
      etcd.vm.box = "coreos-%s" % $update_channel
      etcd.vm.box_version = ">= 1185.3.0"
      etcd.vm.box_url = "http://%s.release.core-os.net/amd64-usr/current/coreos_production_vagrant.json" % $update_channel
  ...
  (1..$controller_count).each do |i|
    config.vm.define vm_name = "c%d" % i do |controller|
      ...
      controller.vm.box = "coreos-%s" % $update_channel
      controller.vm.box_version = ">= 1185.3.0"
      controller.vm.box_url = "http://%s.release.core-os.net/amd64-usr/current/coreos_production_vagrant.json" % $update_channel
      ...
      $forwarded_ports_controller.each do |guest, host|
        controller.vm.network "forwarded_port", guest: guest, host: host, auto_correct: true
      end
  ...
  (1..$worker_count).each do |i|
    config.vm.define vm_name = "w%d" % i do |worker|
      ...
      worker.vm.box = "coreos-%s" % $update_channel
      worker.vm.box_version = ">= 1185.3.0"
      worker.vm.box_url = "http://%s.release.core-os.net/amd64-usr/current/coreos_production_vagrant.json" % $update_channel
      ...
      $forwarded_ports_worker.each do |guest, host|
        worker.vm.network "forwarded_port", guest: guest, host: host, auto_correct: true
      end
      ...
      worker.vm.provision "file", source: "./kubernetes/docker-reg-authentication/config.json", destination: "~/.docker/config.json"

      $shared_folders.each_with_index do |(host_folder, guest_folder), index|
        worker.vm.synced_folder host_folder.to_s, guest_folder.to_s, id: "core-share%02d" % index, :nfs => true, mount_options: ['nolock,vers=3,tcp,noatime,actimeo=1']
      end
    end
  end
